...
import { Shield, Server, Zap, Globe, ArrowRight, Activity, Users, Radio, Music2 } from "lucide-react";
...
export default function HomePage() {
  ...
  const totalPlayers = nodes.reduce((acc, node) => {
    if (!node.info?.stats?.players && !node.connections) return acc;
    const playing = parseInt(node.connections?.split('/')[0] || 0);
    return acc + playing;
  }, 0);
